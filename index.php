...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Authorized users
    $users = [
        'admin@academify.xyz' => ':;(Balu/971):;'
    ];

    if (isset($users[$email]) && $users[$email] === $password) {
        $_SESSION['academify_auth'] = true;
        $_SESSION['academify_user'] = $email;
        setcookie('academify_auth', 'granted', time() + 86400 * 30, '/');
...
if (isset($_SESSION['academify_auth']) && $_SESSION['academify_auth'] === true) {
    header('Location: landing.html');
    exit;
}
?>
...
.error-msg { color: #ef4444; font-size: 0.8rem; margin-top: 0.75rem; }
